...
// src/UploadPrescription.js
import React, { useState } from 'react';
import * as XLSX from 'xlsx';
import styles from './UploadPrescription.module.css';
import { Table, Button } from 'react-bootstrap';

const UploadPrescription = ({ backendUrl }) => {
  const [file, setFile] = useState(null);
  const [errorMessage, setErrorMessage] = useState('');
  const [prescriptionData, setPrescriptionData] = useState([]);
  const [headers, setHeaders] = useState([]);
  ...
  const uploadPrescription = async (prescription) => {
    try {
      await fetch(`${backendUrl}/prescriptions/upload`, {
        method: 'POST',
        headers: {
  ...
  const renderCheckbox = (value) => {
    return <input type="checkbox" checked={value === 'TRUE' || value === 'true'} disabled />;
  };
              ...
              <tbody>
                {prescriptionData.slice(1).map((row, rowIndex) => (
                  <tr key={rowIndex}>
                    {row.map((cell, colIndex) => (
                      <td key={colIndex}>
                        {cell === 'FALSE' || cell === 'TRUE' ? renderCheckbox(cell) : cell}
                      </td>
                    ))}
                  </tr>
                ))}
